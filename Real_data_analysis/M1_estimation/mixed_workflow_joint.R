...
#### Mixed effect model inference (with burn-in handled correctly)
...
mixed.workflow.joint <- function(Y.obs.ord, X.obs.ord, obs.locs.ord, m, N.obs, p, q,
  ...
  ## Prediction storage (ONLY post-burnin)
  ...
  for (i in 1:(q.subsets - 2)) {
    ...
    cond.elpd[counter] <- elpd.joint - sum(elpd.comp[ind])
    counter <- counter + 1
  }
