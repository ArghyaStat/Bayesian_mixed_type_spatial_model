...
mcmc.main <- function(Y.obs.ord, Y.sc.obs.ord, X.obs.ord, obs.locs.ord, m, N.obs, p, q,
  ...
  for (iter in 1:niters) {
    ...
    W.update.details <- update.W(W.obs.ord, beta, chol.Sigma.inv, chol.K.obs.ord.inv,
                                 Y.obs.ord, Y.sc.obs.ord, X.obs.ord, N.obs, log.like, family)
    
    W.obs.ord <- W.update.details$W.obs.ord
    log.like <- log.like + W.update.details$log.like
    log.like.sc <- log.like.sc + W.update.details$log.like.sc
  ...
  theta.chain <- list("W.ord.samples" = W.obs.ord.chain,
                      ...
                      "log.like.sc.mean" = log.like.sc/(q*niters),
