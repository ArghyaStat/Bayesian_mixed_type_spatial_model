mixed.workflow.joint <- function(Y.obs.ord, X.obs.ord, obs.locs.ord, m, N.obs, p, q,
  ...
  for (iter in 1:niters) {
    ...
    ## --- W update ---
    W.obs.ord.details <- update.W(W.obs.ord, beta, Sigma, B, r, NNarray.obs,
                          Y.obs.ord, X.obs.ord, N.obs, log.like, family,
                          family.par = NULL,
                          link = NULL,
                          acc.W, tuning.W)
    
    W.obs.ord <-  W.obs.ord.details$W.obs.ord
    acc.W <- W.obs.ord.details$acc.W
