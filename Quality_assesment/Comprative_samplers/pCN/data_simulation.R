sim.data <- function(
  ...
  if (pred.prop == 0) {
    ...
    pred.indices <- integer(0)
    
    pred.locs <- matrix(numeric(0), nrow = 0, ncol = d)
    obs.locs  <- locations
    
    X.obs  <- X
    X.pred <- matrix(numeric(0), nrow = 0, ncol = p)
    
    Y.obs       <- Y
    Y.pred.true <- matrix(numeric(0), nrow = 0, ncol = q)
    
    true.W.obs <- true.W
    
    ## diameter (safe)
    distmat.obs <- as.matrix(dist(obs.locs))
    diameter <- if (length(distmat.obs) > 0) max(distmat.obs) else 0
    
    joint.locs <- obs.locs   ## 🔥 IMPORTANT
