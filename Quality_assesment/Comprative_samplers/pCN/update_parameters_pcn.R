update.W <- function(W.obs.ord, beta, Sigma, B, r, NNarray.obs,
  ...
  ## ---- draw base Vecchia Gaussian once ----
  Z <- matrix(rnorm(N.obs * q), N.obs, q)
  nu.base <- vecchia_prior_draw(Z, NNarray.obs, B, r)
  
  for (j in seq_len(q)) {
    
    ind.minus <- setdiff(seq_len(q), j)
    
    ## ---- conditional mean and variance (same as ESS) ----
    ...
    eps.j <- sqrt(Sigma.cond) * nu.base[, j]
    ...
    can.W.obs.ord[, j] <-
      mu.j +
      sqrt(1 - tuning.W[j]^2) * (W.obs.ord[, j] - mu.j) +
      tuning.W[j] * eps.j
