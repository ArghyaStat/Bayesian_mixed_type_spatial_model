...
mixed.workflow.joint <- function(Y.obs.ord, X.obs.ord, obs.locs.ord, m, N.obs, p, q,
  ...
  for (iter in 1:niters) {
    ...
    est_time_total <- est_time_total +
      as.numeric(difftime(est_end, est_start, units = "secs"))
    ...
    if (iter %% ceiling(niters / 10) == 0) {
      ...
      cat(sprintf(
        "Estimation Progress: %3.0f%% | Last 10%% Est time: %.4f secs\n",
        100 * iter / niters,
        est_block_time
      ))
    ...
    pred_time_total <- pred_time_total +
      as.numeric(difftime(pred_end, pred_start, units = "secs"))
    ...
    if (iter %% ceiling(niters / 10) == 0) {
      ...
      cat(sprintf(
        "Prediction Progress: %3.0f%% | Last 10%% Pred time: %.4f secs\n",
        100 * iter / niters,
        pred_block_time
      ))
  ...
  total_time <- as.numeric(
    difftime(end_total_time, start_total_time, units = "secs")
  )
  ...
  cat(sprintf("Estimation time (minutes): %.4f\n", est_time_total))
  cat(sprintf("Prediction time (minutes): %.4f\n", pred_time_total))
  cat(sprintf("Total time (minutes): %.4f\n", total_time))
  
  
  cat(sprintf("Acceptance.phi = %.4f\n", acc.phi / niters))
