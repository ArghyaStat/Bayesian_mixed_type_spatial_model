...
update.Sigma <- function(UW,
  ...
  if (family == "Binomial") {
    ...
    chol.Sigma <- sqrt(Sigma)
    
    return(list(
      Sigma = Sigma,
      chol.Sigma = chol.Sigma,
      chol.Sigma.inv = 1 / chol.Sigma
    ))
  }
  ...
  rss.beta.prior <- chol.prec.V.prior %*% M.prior
  rss.beta.post  <- chol.V.tilde %*% M.tilde.part
  
  #### scalar components ####
  
  c.val <- sum(UW^2) +
    sum(rss.beta.prior^2) -
    sum(rss.beta.post^2)
  
  #### IG posterior parameters ####
  
  shape <- a.prior + N.obs / 2
  rate  <- b.prior + 0.5 * c.val
