...
log.likelihood <- function(W, Y, family) {
  # Validate inputs
  if (ncol(W) != ncol(Y)) {
    stop("The dimensions of W and Y must match.")
  }
  if (ncol(Y) != length(family)) {
    stop("The number of columns in Y must match the length of the 'family' vector.")
  }
  ...
  for (j in 1:q) {
    if (family[j] == "Gaussian") {
      ...
      like.total <- like.total + sum(dinvgauss(Y[, j], mean = mu, shape = lambda, log = TRUE))
    } else if (family[j] == "Tweedie") {
      require(tweedie)
      p <- 1.5  # Assuming power parameter; adjust as needed
      phi <- 1  # Dispersion parameter; adjust as needed
      like.total <- like.total + sum(dtweedie(Y[, j], power = p, mu = exp(W[, j]), phi = phi, log = TRUE))
    } else if (family[j] == "Multinomial") {
      require(nnet)
      prob <- exp(W[, j]) / sum(exp(W[, j]))
      like.total <- like.total + sum(dmultinom(Y[, j], prob = prob, log = TRUE))
    } else {
...
#     dims <- dim(chain[[1]])
#     
#     # Handle 1x1 matrix case explicitly
#     if (all(dims == c(1, 1))) {
#       chain_array <- array(unlist(chain), dim = c(1, 1, n_iter))
#     } else {
#       chain_array <- simplify2array(chain)
#     }
#     
#     post.mean <- Reduce("+", chain) / n_iter
#     post.median <- apply(chain_array, c(1, 2), median)
#     post.sd <- sqrt(Reduce("+", lapply(chain, function(x) (x - post.mean)^2)) / n_iter)
#     ci.lower <- apply(chain_array, c(1, 2), quantile, probs = 0.025)
#     ci.upper <- apply(chain_array, c(1, 2), quantile, probs = 0.975)
#     coverage <- (true_value >= ci.lower) & (true_value <= ci.upper)
#     rmse <- sqrt(Reduce("+", lapply(chain, function(x) (x - true_value)^2)) / n_iter)
#     
#   } else {
#     unlisted_chain <- unlist(chain)
#     post.mean <- mean(unlisted_chain)
#     post.median <- median(unlisted_chain)
#     post.sd <- sd(unlisted_chain)
#     ci.lower <- quantile(unlisted_chain, probs = 0.025)
#     ci.upper <- quantile(unlisted_chain, probs = 0.975)
#     coverage <- (true_value >= ci.lower) & (true_value <= ci.upper)
#     rmse <- sqrt(mean((unlisted_chain - true_value)^2))
#   }
#   
...
summary_stats <- function(chain, true_value) {
  ...
  if (is.matrix(chain[[1]])) {
    ...
    # If simplify2array collapses to vector or 2D matrix, fix shape
    if (is.null(dim(arr))) {
      # Scalar case: make (1,1,n_iter) array
      arr <- array(arr, dim = c(1, 1, n_iter))
    } else if (length(dim(arr)) == 2) {
      # Matrix case with simplify2array collapse: add 3rd dim
      arr <- array(arr, dim = c(dim(chain[[1]]), n_iter))
    }
    
    post.median <- apply(arr, c(1, 2), median)
    post.sd <- sqrt(Reduce("+", lapply(chain, function(x) (x - post.mean)^2)) / n_iter)
    ci.lower <- apply(arr, c(1, 2), quantile, probs = 0.025)
    ci.upper <- apply(arr, c(1, 2), quantile, probs = 0.975)
    coverage <- (true_value >= ci.lower) & (true_value <= ci.upper)
    rmse <- sqrt(Reduce("+", lapply(chain, function(x) (x - true_value)^2)) / n_iter)
  } else {
    vec <- unlist(chain)
    post.mean <- mean(vec)
    post.median <- median(vec)
    post.sd <- sd(vec)
    ci.lower <- quantile(vec, probs = 0.025)
    ci.upper <- quantile(vec, probs = 0.975)
    coverage <- (true_value >= ci.lower) & (true_value <= ci.upper)
    rmse <- sqrt(mean((vec - true_value)^2))
  }
...
combine_stats <- function(stats.list) {
  fields <- names(stats.list[[1]])
  combined <- list()
  
  for (field in fields) {
    pieces <- lapply(stats.list, function(x) x[[field]])
    
    if (is.matrix(pieces[[1]])) {
      # Combine as 3D array or bind as matrix (e.g., p x q)
      combined[[field]] <- do.call(cbind, pieces)  # matrix of (p x q)
    } else if (is.vector(pieces[[1]]) && length(pieces[[1]]) > 1) {
      # Combine vectors as matrix
      combined[[field]] <- do.call(cbind, pieces)  # matrix of (len x q)
    } else {
      # Combine scalars as a row vector
      combined[[field]] <- unlist(pieces)  # vector of length q
    }
  }
