...
results <- foreach(r = 1:reps, .packages = libraries) %dopar% {
  ...
  #### Prior specifications ####
    ...
    predictive.results <- predictive.samples(W.ord.post, beta.post, chol.Sigma.post, phi.post, 
                                             nu, m,
                                             X.obs.ord, X.pred.ord, distlocs.nn, NNarray.all,
                                             N.obs, N.pred, pred.iters, p, 
                                             family = sub_family)
    
    Y.pred.sep <- Map(function(mat, col) {
      mat[, j] <- col
      mat
    }, Y.pred.sep, predictive.results$Y.pred.ord)
   ...
   log.pred.sep <- predictive.results$log.pred.sep
   ...
   elpd_j <- m + log(mean(exp(log.pred.sep - m)))
...
list.save(results, file = "gp_strong_sep_dep100.Rdata")
