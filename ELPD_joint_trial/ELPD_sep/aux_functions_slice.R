...
mlpd <- function(W.obs.ord.samples, Y.pred.ord.true, family) {
  ...
  # Log joint predictive density for each MCMC iteration
  log_joint_densities <- numeric(niters)
    ...
    for (k in 1:N.pred) {
      for (j in 1:q) {
        ...
        log_dens <- switch(family[j],
                           ...
                           "Negative-Binomial" =
                             MASS::dnbinom(y, mu = exp(mu), size = 1, log = TRUE),
                           "Inverse-Gaussian" = {
                             if (!requireNamespace("statmod", quietly = TRUE))
                               stop("Please install 'statmod'.")
                             statmod::dinvgauss(y, mean = exp(mu), shape = 1, log = TRUE)
                           },
                           "Tweedie" = {
                             if (!requireNamespace("tweedie", quietly = TRUE))
                               stop("Please install 'tweedie'.")
                             tweedie::dtweedie(y, mu = exp(mu), phi = 1, power = 1.5, log = TRUE)
                           },
                           stop(paste("Unsupported family:", family[j]))
        )
        
        log_joint_density <- log_joint_density + log_dens
      }
    }
    
    log_joint_densities[i] <- log_joint_density
  }
  
  # log(mean(exp(log_joint_densities))) in a stable way
  max_log <- max(log_joint_densities)
  max_log + log(mean(exp(log_joint_densities - max_log)))
}
